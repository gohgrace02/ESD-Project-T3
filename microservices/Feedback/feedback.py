...
class Feedback(db.Model):
    ...
    def __init__(self, user_id, project_id, rating, feedback_info, submitted_at):
    ...
    def json(self):
        return {
            'feedback_id': self.feedback_id, 'user_id': self.user_id, 'project_id': self.project_id, 'rating': self.rating, 'feedback_info': self.feedback_info, 'submitted_at': self.submitted_at
        }
...
def create_feedback(project_id):
    ...
    feedback = Feedback(user_id=data.get('user_id'), project_id=project_id, rating=data.get('rating'), feedback_info=data.get('feedback_info'), submitted_at=data.get('submitted_at')) 
    print(data)
